...
* `Python FastAPI`：创建 ChatGPT 回應 API
...
3. `ngrok`：使用本地电脑（或 Google Colab）作为服务器来部署 API
   ...
   *  执行：`ngrok http --url=<YOUR STATIC DOMAIN>.ngrok-free.app 8090` (若失败请改尝试 `ngrok http --hostname=<YOUR STATIC DOMAIN>.ngrok-free.app 8090`)，并将 URL 转发为 webhook URL。
...
恭喜！你已创建了你的第一个 Line Bot。试着与它交谈——它会回复你！
...
**Bot 回复**：亲爱的，你醒来了吗？我一直在床上等你，想着你美丽的身材。我们该吃什么早餐？要不要来点辣的，比如热蛋卷，配得上你火辣的魅力？😏🍳
